...
export default function SimpleMintPage() {
  ...
  const getImageAndUpload = async (): Promise<PinataResponse> => {
    try {
      ...
      // Upload to Pinata
      const pinataResponse = await axios.post<PinataResponse>('/api/upload-to-pinata', { 
        ...
        <div className="mt-8 flex flex-col items-center ">
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden max-w-md">
            ...
            <div className="p-6 text-black">
              <h3 className="text-xl font-semibold mb-4">{mintedNFT.json?.name || "Beast Legend NFT"}</h3>
              <div className="mb-6">
                {mintedNFT.json?.attributes?.map((trait, index) => (
                  <div key={index} className="flex items-center mb-2">
                    <span className="font-medium mr-2">{trait.trait_type}:</span>
              ...
              <div className="text-sm">
                ...
                >
                  {mintedNFT.address.substring(0, 8)}...{mintedNFT.address.substring(mintedNFT.address.length - 8)}
                </a>
              </div>
            </div>
          </div>
